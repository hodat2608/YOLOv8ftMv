...
class Model_Camera_2(Base, MySQL_Connection, PLC_Connection):
    ...
    def read_plc_value_from_file(self):
        with open(
            r"C:\Users\CCSX009\Documents\ultralytics-main\var_plc2.txt", "r"
        ) as file:
            content = file.read()
            parts = content.split("=")
            if len(parts) == 2:
                ...
                raise ValueError(
                    "File format is incorrect. Expected 'read_plc = <value>' format."
                )

    def write_plc_value_to_file(self):
        with open(
            r"C:\Users\CCSX009\Documents\ultralytics-main\var_plc2.txt", "w"
        ) as file:
    ...
    def extract(self):
        ...
        if self.img_buffer == []:
            pass
        image_result, results_detect, list_label_ng, valid = self.process_image_func(
            self.img_buffer[0], width, height
        )
    ...
    def extract_fh(self):
        ...
        image_paths = glob.glob(
            f"C:/Users/CCSX009/Documents/yolov5/test_image/camera2/*.jpg"
        )
        if len(image_paths) == 0:
            ...
            for filename in image_paths:
                ...
                for widget in self.img_frame.winfo_children():
                    widget.destroy()
                image_result, results_detect, list_label_ng = self.process_image_func(
                    img1_orgin, width, height
                )
                self.result_detection.config(
                    text=results_detect, fg="green" if results_detect == "OK" else "red"
                )
