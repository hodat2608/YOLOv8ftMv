...
class base_handle_video(PLC_Connection, MySQL_Connection):
    ...
    def select_video(self, path_video):
        selected_file = filedialog.askopenfilename(
            title="Choose a file", filetypes=[("Video Files Type", "*.mp4")]
        )
        if selected_file:
    ...
    def update_progress(self, current_frame, total_frames, progress_label):
        ...
        progress_label.config(
            text=f"Rendering on {device} in progress: {progress:.2f}%"
        )
        progress_label.update_idletasks()
    ...
    def handle_params_video(self, frame, classes, line_thickness, names):
        ...
        if results[0].boxes.id is not None:
            ...
            for box, track_id, cls, xywh, conf in zip(
                boxes, track_ids, clss, xywhs, conf_list
            ):
                ...
                if (
                    a_point[0] <= xywh[0] <= d_point[0]
                    and a_point[1] <= xywh[1] <= d_point[1]
                ):
            ...
            if len(coun_id) > 2:
                cv2.putText(
                    frame, "NG", (200, 200), cv2.FONT_HERSHEY_COMPLEX, 3, (0, 0, 255), 5
                )
            else:
                cv2.putText(
                    frame, "OK", (200, 200), cv2.FONT_HERSHEY_COMPLEX, 3, (0, 255, 0), 5
                )
