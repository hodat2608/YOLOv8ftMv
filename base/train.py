...
class Training_Data(Base):
    ...
    def layout_basic(self):
        ...
        scrollbar_frame_1 = tk.Scrollbar(
            Frame_1, orient="vertical", command=canvas.yview
        )
        ...
        ttk.Label(
            source_FOLDER, text="Source folder:", font=("ubuntu", 12), width=15
        ).grid(column=0, row=1, padx=10, pady=5, sticky="w")
        ...
        self.source_FOLDER_entry_btn.grid(
        ...
        ttk.Label(
            source_CLASS, text="Source class:", font=("ubuntu", 12), width=15
        ).grid(column=0, row=1, padx=10, pady=5, sticky="w")
        ...
        self.source_CLASS_entry_button.grid(
            row=1, column=2, padx=(0, 10), pady=3, sticky="w", ipadx=5, ipady=1
        )
        ...
        ttk.Label(time_frame, text="Times: ", font=("ubuntu", 12), width=15).grid(
            column=0, row=1, padx=10, pady=5, sticky="w"
        )

        self.time_frame_entry = ttk.Entry(time_frame, width=45)
        self.time_frame_entry.grid(
            row=1, column=1, padx=(0, 10), pady=3, sticky="w", ipadx=5, ipady=2
        )
        ...
        ttk.Label(
            source_save_result, text="Save Results:", font=("ubuntu", 12), width=15
        ).grid(column=0, row=1, padx=10, pady=5, sticky="w")

        self.source_save_result_entry = ttk.Entry(source_save_result, width=45)
        self.source_save_result_entry.grid(
            row=1, column=1, padx=(0, 10), pady=3, sticky="w", ipadx=5, ipady=2
        )

        self.source_save_result_entry_button = tk.Button(
            source_save_result, text="Browse...", command=lambda: self.browse_folder2()
        )
        self.source_save_result_entry_button.grid(
            row=1, column=2, padx=(0, 10), pady=3, sticky="w", ipadx=5, ipady=1
        )
        ...
        ttk.Label(
            name_results, text="Name Results:", font=("ubuntu", 12), width=15
        ).grid(column=0, row=1, padx=10, pady=5, sticky="w")

        self.name_results_entry = ttk.Entry(name_results, width=45)
        self.name_results_entry.grid(
            row=1, column=1, padx=(0, 10), pady=3, sticky="w", ipadx=5, ipady=2
        )
        ...
        ttk.Checkbutton(save_frame, variable=self.save).grid(
        ...
        ttk.Spinbox(
            workers_frame, from_=0, to=32, textvariable=self.workers, width=7
        ).grid(
            row=1,
            column=2,
            columnspan=2,
        ...
        ttk.Label(
            optimizer_frame, text="Optimizer:", font=("ubuntu", 12), width=15
        ).grid(row=1, column=1, padx=10, pady=5, sticky="w")
        self.optimizer_model = ttk.Combobox(
            optimizer_frame, values=["SGD", "Adam", "AdamW", "auto"], width=7
        )
        self.optimizer_model.grid(
            row=1,
            column=2,
            columnspan=2,
            padx=(0, 10),
        ...
        ttk.Label(
            pretrained_frame, text="Pretrained:", font=("ubuntu", 12), width=15
        ).grid(row=1, column=1, padx=10, pady=5, sticky="w")
        self.pretrained = tk.BooleanVar(value=True)
        ttk.Checkbutton(pretrained_frame, variable=self.pretrained).grid(
            row=1, column=2, sticky="w"
        )
        ...
        ttk.Checkbutton(verbose_frame, variable=self.verbose).grid(
            row=1, column=2, sticky="w"
        )
        ...
        ttk.Label(
            deterministic_frame, text="deterministic:", font=("ubuntu", 12), width=15
        ).grid(row=1, column=1, padx=10, pady=5, sticky="w")
        self.deterministic = tk.BooleanVar(value=True)
        ttk.Checkbutton(deterministic_frame, variable=self.deterministic).grid(
            row=1, column=2, sticky="w"
        )
        ...
        ttk.Label(
            single_cls_frame, text="single_cls:", font=("ubuntu", 12), width=15
        ).grid(row=1, column=1, padx=10, pady=5, sticky="w")
        self.single_cls = tk.BooleanVar(value=True)
        ttk.Checkbutton(single_cls_frame, variable=self.single_cls).grid(
            row=1, column=2, sticky="w"
        )
        ...
        ttk.Checkbutton(rect_frame, variable=self.rect).grid(
            row=1, column=2, sticky="w"
        )
        ...
        ttk.Label(
            close_mosaic, text="close_mosaic:", font=("ubuntu", 12), width=15
        ).grid(row=1, column=1, padx=10, pady=5, sticky="w")

        optionsbatch = [10, 20, 30, 40, 50, 100]
        self.close_mosaic_model = ttk.Combobox(
            close_mosaic, values=optionsbatch, width=7
        )
        self.close_mosaic_model.grid(
            row=1,
            column=2,
            columnspan=2,
            padx=(0, 10),
        ...
        ttk.Checkbutton(fraction_frame, variable=self.fraction).grid(
            row=1, column=2, sticky="w"
        )
        ...
        ttk.Checkbutton(profile_frame, variable=self.profile).grid(
            row=1, column=2, sticky="w"
        )
        ...
        self.excute_button.grid(
            row=1, column=2, padx=(0, 10), pady=3, sticky="w", ipadx=15, ipady=1
        )

        progress_label = ttk.Label(excute, text="", font=("Segoe UI", 12))
        progress_label.grid(
            row=1, column=3, columnspan=2, padx=10, pady=5, sticky="nws"
        )
    ...
    def browse_folder1(self):
        file_selected = filedialog.askopenfilename(
            title="Select classes.txt file", filetypes=(("Text files", "classes.txt"),)
        )
        if file_selected:
    ...
    def run_h(self, progress_label):
        if (
            ...
            or self.source_CLASS_entry.get() == None
            or self.source_CLASS_entry.get() == ""
        ):
            ...
            try:
                ...
                messagebox.showwarning(
                    "Warning", f"Can not create *train and ** valid path. Error: {e}"
                )
            ...
            for index, file_path in enumerate(jpg):
                ...
                shutil.copyfile(
                    label_path, os.path.join(dest_label_folder, file_name[:-3] + "txt")
                )
                progress_retail = (index + 1) / total_files * 100
                progress_label.config(
                    text=f"Split datasets in progress: {progress_retail:.2f}%"
                )
    ...
    def Excute(self, progress_label):
        ...
        if dataset_format == "" or dataset_format == "None":
            ...
            if not txt_files:
                messagebox.showwarning(
                    "Warning", "No .txt files found in the source folder."
                )
                return
            with open(os.path.join(src_path, txt_files[1]), "r") as file:
                ...
                for line in lines:
                    ...
                    if dataset_format == "HBB Format":
                        if "YOLO_OBB" in line:
                            messagebox.showwarning(
                                "Warning", "Invalid Supported Dataset Formats"
                            )
                            return
                        else:
                            processing_thread = threading.Thread(
                                target=self.run_h, args=(progress_label,)
                            )
                        ...
                        if "YOLO_OBB" not in line:
                            ...
                            processing_thread = threading.Thread(
                                target=self.run_o, args=(progress_label,)
                            )
                            processing_thread.start()
                            return
