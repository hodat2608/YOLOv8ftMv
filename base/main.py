...
class main(): 
    def __init__(self):
        ...
        window.state('zoomed')
        style = ttk.Style()
        style.configure("TNotebook", background="white", foreground="black", font=("Arial", 10, "bold"))
        ...
        exit = asserts.exit
        filemenu.add_command(label=" Open Camera Display",image=Open,compound=tk.LEFT,command=lambda:display_layout(notebook, window))
        filemenu.add_separator()
        filemenu.add_command(label=" Exit",image=exit,compound=tk.LEFT,command=lambda:confirm_exit(window))
