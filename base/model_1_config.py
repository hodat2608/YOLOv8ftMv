...
class Model_Camera_1(PublicClassInitialization,Base_GUI):
    ...
    def check_connection(self):
        connected = False
        if not connected:
            if super().socket_connect(
                self.socket, PLC_CONNECTION["HOST"], PLC_CONNECTION["PORT"]
            ):
                connected = True
    ...
    def clone_device(self):
        ...
        self.writedata(self.socket, PLC_DATASOC["ALREADY"], 1)
        self.dual_Thread()
    ...
    def load_parameters_model(
        ...
        return super().load_parameters_model(
            model1,
            load_path_weight,
            load_item_code,
            load_confidence_all_scale,
    ...
    def dual_submit(self):
        try:
            if self.readdata(
                self.socket, PLC_DATASOC["TRIGGER"]
            ) == 1 or keyboard.is_pressed("shift+a"):
                self.executor.submit(
                    self.request_pylon.TriggerOnce,
                    self.task,
                    PLC_DATASOC["TRIGGER"],
                    PLC_DATASOC["BUSY"],
                    self.socket,
            ...
            self.writedata(self.socket, PLC_DATASOC["ALREADY"], 0)
    ...
    def dual_Thread(self):
        try:
            if self.readdata(
                ...
                threading.Thread(
                    target=self.request_pylon.TriggerOnce(
                        self.task,
                        PLC_DATASOC["TRIGGER"],
                        PLC_DATASOC["BUSY"],
                        self.socket,
    ...
    def callback(self):
        ...
        if not self.task.empty():
            pass
        image_result, results_detect, list_label_ng, valid, time_processing = (
            self.process_image_func(self.task.get(), width, height)
        )
        self.result_detection.config(
            text=results_detect, fg="green" if results_detect == "OK" else "red"
        )
        list_label_ng = ",".join(list_label_ng)
        self.imgs_gui(image_result)
        self.time_processing_output.config(text=f"{time_processing}")
        self.canvas_i.create_text(
            10,
            10,
            anchor=tk.NW,
        ...
        self.canvas_i.create_text(
            ...
            40,
            anchor=tk.NW,
            text=f"Result: {results_detect}",
            fill="green" if results_detect == "OK" else "red",
            font=("Segoe UI", 20),
        )
        self.canvas_i.create_text(
            10,
    ...
    def callback_fh(self):
        ...
        height = 800
        t1 = time.time()
        image_paths = glob.glob(f"C:/ultralytics-main/datasets/*.jpg")
        ...
        for filename in image_paths:
            ...
            for widget in self.img_frame.winfo_children():
                widget.destroy()
            image_result, results_detect, list_label_ng, valid, _ = (
    ...
    def layout_camframe_default(self):
        ...
        frame = ttk.LabelFrame(
            content_frame, width=900, height=900, style="Custom.TLabelframe"
        )
        ...
        time_frame = ttk.LabelFrame(
            frame, text=f"Time Processing Camera", width=300, height=100
        )
    ...
    def configuration_frame(self):
        ...
        self.model = self.torch_load_nodemap(
            source=load_path_weight, task="detect", device=self.device
        )
        self.load_first_img()
        ...
        scrollbar_x = ttk.Scrollbar(
            configuration_frame_tab, orient="horizontal", command=canvas1.xview
        )
        ...
        canvas1.bind_all(
            "<MouseWheel>",
            lambda event: canvas1.yview_scroll(int(-1 * (event.delta / 120)), "units"),
        )
        ...
        Frame_2 = ttk.LabelFrame(
            scrollable_frame, text="Pamameters", width=frame_width, height=frame_height
        )
        ...
        self.datasets_format_model.bind(
            "<<ComboboxSelected>>", partial(self.on_option_change, Frame_2=Frame_2)
        )
    ...
    def option_layout_models(self, Frame_1, Frame_2, records):
        ...
        datasets_format.grid(
            row=1, column=0, columnspan=2, padx=(15, 30), pady=10, sticky="w"
        )

        ttk.Label(
            datasets_format, text="Dataset Formats:", font=("ubuntu", 12), width=15
        ).grid(column=0, row=1, padx=10, pady=5, sticky="w")

        option_datasets_format = ["HBB", "OBB"]
        self.datasets_format_model = ttk.Combobox(
            datasets_format, values=option_datasets_format, width=7
        )
        self.datasets_format_model.grid(
            row=1, column=2, padx=(0, 10), pady=5, sticky="w", ipadx=40, ipady=2
        )

        datasets_format_model_button = tk.Button(
            datasets_format,
            text="Confirm",
            command=lambda: self.datasets_format_model_confirm(Frame_2),
        )
        datasets_format_model_button.grid(
            row=1, column=3, padx=(0, 8), pady=3, sticky="w", ipadx=5, ipady=2
        )
        datasets_format_model_button.config(state="disabled")
        self.lockable_widgets.append(datasets_format_model_button)

        ttk.Label(Frame_1, text="Model Path", font=("Segoe UI", 12)).grid(
            column=0, row=0, padx=10, pady=5, sticky="nws"
        )
        ...
        button_frame.grid(
            row=3, column=0, columnspan=2, padx=(30, 30), pady=5, sticky="w"
        )

        change_model_button = tk.Button(
            button_frame,
            text="Change Model",
            command=lambda: self.change_model(Frame_2),
        )
        change_model_button.grid(
            row=0, column=0, padx=(0, 8), pady=5, sticky="w", ipadx=5, ipady=2
        )
        change_model_button.config(state="disabled")
        self.lockable_widgets.append(change_model_button)

        load_parameters = tk.Button(
            button_frame,
            text="Load Parameters",
            command=lambda: self.load_parameters_from_weight(records),
        )
        load_parameters.grid(
            row=0, column=1, padx=(0, 8), pady=5, sticky="w", ipadx=5, ipady=2
        )
        ...
        custom_para.grid(
            row=0, column=2, padx=(0, 8), pady=5, sticky="w", ipadx=5, ipady=2
        )
        custom_para.config(state="disabled")
        self.lockable_widgets.append(custom_para)

        self.permisson_btn = tk.Button(
            button_frame,
            text="Unlock",
            command=lambda: self.toggle_state_layout_model(),
        )
        self.permisson_btn.grid(
            row=0, column=3, padx=(0, 8), pady=5, sticky="w", ipadx=5, ipady=2
        )

        label_scale_conf_all = ttk.Label(
            Frame_1, text="Confidence", font=("Segoe UI", 12)
        )
        label_scale_conf_all.grid(
            row=4, column=0, columnspan=2, padx=10, pady=5, sticky="nws"
        )

        self.scale_conf_all = tk.Scale(
            Frame_1, from_=1, to=100, orient="horizontal", length=400
        )
        self.scale_conf_all.grid(
            row=5, column=0, columnspan=2, padx=30, pady=5, sticky="nws"
        )
        self.lockable_widgets.append(self.scale_conf_all)

        label_size_model = ttk.Label(Frame_1, text="Size Model", font=("Segoe UI", 12))
        label_size_model.grid(
            row=6, column=0, columnspan=2, padx=10, pady=5, sticky="nws"
        )

        options = [480, 640, 832, 1024]
        self.size_model = ttk.Combobox(Frame_1, values=options, width=7)
        self.size_model.grid(
            row=7,
            column=0,
            columnspan=2,
            padx=30,
        ...
        name_item_code.grid(
            row=8, column=0, columnspan=2, padx=10, pady=5, sticky="nws"
        )

        self.item_code = ttk.Entry(Frame_1, width=10)
        self.item_code.grid(
            row=9, column=0, columnspan=2, padx=30, pady=5, sticky="w", ipadx=5, ipady=2
        )
        self.lockable_widgets.append(self.item_code)

        save_data_to_database = ttk.Button(
            Frame_1, text="Apply", command=lambda: self.save_params_model()
        )
        save_data_to_database.grid(
            row=10,
            column=0,
            columnspan=2,
            padx=30,
        ...
        camera_frame_display = ttk.Label(
            Frame_1, text="Modify Image", font=("Segoe UI", 12)
        )
        camera_frame_display.grid(
            row=11, column=0, columnspan=2, padx=10, pady=5, sticky="nws"
        )
        ...
        camera_custom_setup.grid(
            row=13, column=0, columnspan=2, padx=(30, 30), pady=5, sticky="w"
        )

        single_img = tk.Button(
            camera_custom_setup,
            text="Only Image",
            command=lambda: self.detect_single_img(camera_frame),
        )
        single_img.grid(
            row=0, column=0, padx=(0, 10), pady=5, sticky="w", ipadx=5, ipady=2
        )
        ...
        multi_img = tk.Button(
            camera_custom_setup,
            text="Multi Image",
            command=lambda: self.detect_multi_img(camera_frame),
        )
        multi_img.grid(
            row=0, column=1, padx=(0, 10), pady=5, sticky="w", ipadx=5, ipady=2
        )
        multi_img.config(state="disabled")
        self.lockable_widgets.append(multi_img)

        previos_img = tk.Button(
            camera_custom_setup,
            text="Prev...",
            command=lambda: self.detect_previos_img(camera_frame),
        )
        previos_img.grid(
            row=0, column=2, padx=(0, 10), pady=5, sticky="w", ipadx=5, ipady=2
        )
        ...
        next_img = tk.Button(
            camera_custom_setup,
            text="Next...",
            command=lambda: self.detect_next_img(camera_frame),
        )
        next_img.grid(
            row=0, column=3, padx=(0, 10), pady=5, sticky="w", ipadx=5, ipady=2
        )
        next_img.config(state="disabled")
        self.lockable_widgets.append(next_img)

        auto_detect = tk.Button(
            camera_custom_setup,
            text="Auto Detect",
            command=lambda: self.detect_auto(camera_frame),
        )
        auto_detect.grid(
            row=0, column=4, padx=(0, 10), pady=5, sticky="w", ipadx=7, ipady=2
        )
        auto_detect.config(state="disabled")
        self.lockable_widgets.append(auto_detect)

        self.make_cls_var = tk.BooleanVar()
        make_cls = tk.Checkbutton(
            camera_custom_setup,
            text="Make class",
            variable=self.make_cls_var,
            onvalue=True,
            offvalue=False,
            anchor="w",
        )
        make_cls.grid(
            row=0, column=5, padx=(0, 10), pady=5, sticky="w", ipadx=2, ipady=2
        )
        ...
        logging_frame.grid(
            row=14, column=0, columnspan=2, padx=(30, 30), pady=10, sticky="w"
        )

        logging = tk.Button(
            logging_frame,
            text="Logging Image",
            command=lambda: self.logging(
                folder_ok,
                folder_ng,
                logging_ok_checkbox_var,
                logging_ng_checkbox_var,
        ...
        logging.grid(
            row=0, column=0, padx=(0, 10), pady=5, sticky="w", ipadx=7, ipady=2
        )
        ...
        percent_entry.grid(
            row=0, column=1, padx=(0, 10), pady=3, sticky="w", ipadx=5, ipady=2
        )

        logging_ok_checkbox_var = tk.BooleanVar()
        logging_ok_checkbox = tk.Checkbutton(
            logging_frame,
            text="OK",
            variable=logging_ok_checkbox_var,
            onvalue=True,
            offvalue=False,
        )
        logging_ok_checkbox.grid(
            row=1, column=2, padx=(0, 10), pady=5, sticky="w", ipadx=7, ipady=2
        )
        ...
        logging_ng_checkbox = tk.Checkbutton(
            logging_frame,
            text="NG",
            variable=logging_ng_checkbox_var,
            onvalue=True,
            offvalue=False,
        )
        logging_ng_checkbox.grid(
            row=2, column=2, padx=(0, 10), pady=5, sticky="w", ipadx=7, ipady=2
        )
        ...
        folder_ok.grid(
            row=1, column=0, padx=(0, 10), pady=3, sticky="w", ipadx=15, ipady=2
        )

        folder_ng = ttk.Entry(logging_frame, width=45)
        folder_ng.grid(
            row=2, column=0, padx=(0, 10), pady=3, sticky="w", ipadx=15, ipady=2
        )

        folder_ok_button = tk.Button(
            logging_frame,
            text="Folder OK",
            command=lambda: self.pick_folder_ok(folder_ok),
        )
        folder_ok_button.grid(
            row=1, column=1, padx=(0, 8), pady=3, sticky="w", ipadx=5, ipady=2
        )
        folder_ok_button.config(state="disabled")
        self.lockable_widgets.append(folder_ok_button)

        folder_ng_button = tk.Button(
            logging_frame,
            text="Folder NG",
            command=lambda: self.pick_folder_ng(folder_ng),
        )
        folder_ng_button.grid(
            row=2, column=1, padx=(0, 8), pady=3, sticky="w", ipadx=5, ipady=2
        )
    ...
    def option_layout_parameters(self, Frame_2, model):
        ...
        joint_detect.grid(
            row=0, column=1, padx=15, pady=5, sticky="w", ipadx=2, ipady=2
        )
        ...
        for i1 in range(len(model.names)):
            ...
            join_checkbox = tk.Checkbutton(
                Frame_2,
                variable=join_checkbox_var,
                onvalue=True,
                offvalue=False,
                anchor="w",
            )
            ...
            ok_checkbox = tk.Checkbutton(
                Frame_2,
                variable=ok_checkbox_var,
                onvalue=True,
                offvalue=False,
                anchor="w",
            )
            ...
            ng_checkbox = tk.Checkbutton(
                Frame_2,
                variable=ng_checkbox_var,
                onvalue=True,
                offvalue=False,
                anchor="w",
            )
            ...
            conf_scale = tk.Scale(
                Frame_2, from_=1, to=100, orient="horizontal", length=250
            )
    ...
    def option_layout_parameters_orient_bounding_box(self, Frame_2, model):
        ...
        for i1 in range(len(model.names)):
            ...
            conf_scale = tk.Scale(
                Frame_2, from_=1, to=100, orient="horizontal", length=150
            )
            row_widgets.append(conf_scale)
            self.conf_scales.append(conf_scale)
            self.lock_params.append(conf_scale)
